...
- Natural Earth 110m land（パブリックドメイン）を Miller 図法で投影し、SVG パス文字列へ変換して埋め込む（約48KB）。
- ポイントの緯度経度は表示時に同じ図法定数（util.js）で投影する。変換スクリプトと定数を共有することで座標系のずれを防ぐ。
- パン・ズームは viewBox の書き換えで実装。マーカー半径はズーム率に応じて再計算し、拡大時はラベルを表示する。
